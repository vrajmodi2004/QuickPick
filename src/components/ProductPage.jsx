          ...
          <div key={item._id} className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <div className="p-6">
              ...
              <button
                ...
                onClick={() => togglePrices(item._id)}
              >
                {showPrices[item._id] ? "Hide Prices" : "Show Prices"}
              </button>
              
              {showPrices[item._id] && (
                <div className="mt-4 space-y-2">
                  {Object.entries(item.prices).map(([platform, price]) => (
                    <div key={platform} className="flex justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
